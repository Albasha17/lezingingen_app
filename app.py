...
if basis_vraag == "Nee":
    ...
    with st.form("registration_form"):
        ...
        if st.form_submit_button("Bevestig Aanmelding"):
            if not vn or not an or not email: st.error("Vul alle velden in.")
            else:
                try:
                    ...
                    st.success(f"✅ Bedankt {vn}! Je staat op de lijst voor **{msg_k}**. Tot **{datum_zonder_nul}** (aanvang **{msg_t.strftime('%H:%M')}** bij **[{msg_l}]({msg_u})**).")
                    ...
                    if "@" in email:
                        if send_confirmation_email(email, vn, att_type, join_din, f"EU Studiegroep {maand_naam.capitalize()} {EVENT_DATE.year} Bevestiging", g_url, i_dat):
                            st.info(f"📧 Bevestiging verstuurd naar {email}")
                    
                    st.divider()
                    c_g, c_i = st.columns(2)
                    with c_g: st.markdown(f'''<a href="{g_url}" target="_blank"><button style="width:100%; background-color:#4285F4; color:white; border:none; padding:10px; border-radius:5px; font-weight:bold; cursor:pointer;">Google Agenda ↗</button></a>''', unsafe_allow_html=True)
                    with c_i: st.download_button("Download Outlook / iCal 📥", i_dat, "lezing.ics", "text/calendar", use_container_width=True)
                except Exception as e: st.error(f"Fout: {e}")
